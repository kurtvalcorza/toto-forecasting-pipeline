...
def validate_model_card() -> None:
    ...
    assert not FORBIDDEN.search(text), (
        "MODEL_CARD.md contains placeholder/scaffolding text"
    )
    ...
    for heading in REQUIRED_CARD_HEADINGS:
        ...
        assert len(matches) == 1, (
            f"required model-card heading missing/duplicated: {heading}"
        )
...
def _compile_notebook(path: Path) -> None:
    ...
    assert profile in ALLOWED_PROFILES, (
        f"{path}: missing/invalid metadata.dimer.notebook_profile"
    )
    assert not FORBIDDEN.search(source_text), f"{path}: placeholder text found"
    ...
    for index, cell in enumerate(notebook.get("cells", [])):
        ...
        try:
            ...
            raise AssertionError(
                f"{path}: code cell {index} does not compile: {exc}"
            ) from exc
